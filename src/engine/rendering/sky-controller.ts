...
export class SkyController {
  ...
  /** Cool moon fill light at night (driven by night factor in update()). */
  ...
  /** Register a mesh with noa's octree so it actually renders. */
  private register(mesh: BABYLON.AbstractMesh): void {
    if (this.registerWithOctree) {
      try { this.registerWithOctree(mesh); } catch (e) {
  ...
  private buildCelestialSphere(
    ...
    this.register(mesh); // CRITICAL: register with noa octree or it never renders
    return { mesh, mat };
  }
  ...
  public update(
    ...
    // Moon fill light: driven by night factor.
    if (this.moonLight) {
